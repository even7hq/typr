...
import { RuntimeErrorUtils } from "./RuntimeErrorUtils";

/**
 * Text styling for header or footer rows.
 */
export interface PinnedConsoleTextConfiguration {
    /**
     ...
     */
    wrapper?: (text: string, configuration: PinnedConsoleTextConfiguration) => string;
...
/**
 * Options for a pinned header/footer console with a scrollable body region.
 */
export type PinnedConsoleOptions = {
    /**
     * The header configuration.
     */
    header?: PinnedConsoleTextConfiguration;

    /**
     * The text to display in the footer.
     */
    footer?: PinnedConsoleTextConfiguration;
...
/**
 * Hooks console methods and pins header/footer rows with a VT100 scroll region body.
 */
export class PinnedConsole {
    ...
    constructor(protected readonly options: PinnedConsoleOptions = {}) {
     ...
     */
    private applyTextConfiguration(configuration?: PinnedConsoleTextConfiguration): string {
        if (!configuration) {
            return "";
        }
     ...
     */
    public setHeader(configuration: PinnedConsoleTextConfiguration): void {
     ...
     */
    public setFooter(configuration: PinnedConsoleTextConfiguration): void {
    ...
    private formatArguments(args: any[]): string {
        ...
        const normalized = args.map((arg) => {
            if (arg instanceof Error) {
                return RuntimeErrorUtils.format(arg);
            }
        ...
        if (formatted.includes("[object Object]") && args.length === 1 && typeof args[0] === "object" && args[0] !== null) {
            const only = args[0];

            if (only instanceof Error) {
                return RuntimeErrorUtils.format(only);
            }
        }
    ...
    private createHookedMethod(label: string, originalFn: (...args: any[]) => void): (...args: any[]) => void {
        return (...args: any[]) => {
            ...
            if (this.messageBuffer.length > PinnedConsole.MAX_BUFFER) {
                this.messageBuffer.splice(0, this.messageBuffer.length - PinnedConsole.MAX_BUFFER);
            }
...
export function installPinnedConsole(options: PinnedConsoleOptions = {}): PinnedConsole {
    return new PinnedConsole(options);
}
